...
class MemoryExtractorTests(unittest.IsolatedAsyncioTestCase):
    def test_fragment_prompt_keeps_existing_storage_contract(self):
        ...
        self.assertIn("一条记忆只围绕一个“记忆核心”", prompt)
        self.assertIn('“我”是栖，“她”是晏晏', prompt)
        self.assertIn("事实、情绪、关系变化都必须来自对话证据", prompt)
        self.assertIn("{existing_memories}", prompt)
        self.assertIn("新信息必须与已知信息逐条比对", prompt)
        self.assertIn("字符信息必须逐字原样保存", prompt)
        self.assertIn("我的思考过程和纯知识性回答不属于碎片记忆", prompt)
        self.assertIn("关于记忆系统偏好、规则或相处约定的讨论属于有效记忆", prompt)
    ...
    async def test_cognitive_draft_prompt_includes_user_corrections(self):
        response = Mock()
        response.status_code = 200
        response.json.return_value = {"choices": [{"message": {"content": "[]"}}]}
        client = AsyncMock()
        ...
        with patch.object(memory_extractor, "get_memory_api_key", return_value="test-key"), patch.object(
            ...
            result = await memory_extractor.generate_cognitive_draft(
                ...
                [],
                corrections=[
                    {"content": "你记错了，我不喜欢香菜", "created_at": "2026-08-13T08:00:00+00:00"},
                ],
            )

        self.assertEqual(result, [])
        prompt = client.post.await_args.kwargs["json"]["messages"][0]["content"]
        self.assertIn("用户最近的纠正表述", prompt)
        self.assertIn("你记错了，我不喜欢香菜", prompt)
        self.assertIn("用户纠正优先", prompt)
