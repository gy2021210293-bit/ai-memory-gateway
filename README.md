...
- **实体概况编辑** — 可直接手动修正摘要、关系、稳定事实、近期动态、偏好、待确认信息与证据记忆，也可先让模型生成草稿再逐项修改确认
...
- **Dashboard 全面升级** — 分层 Tab 标签页（全部/核心/事件/碎片 + 计数）、层级下拉选择器、标题编辑、无需横向滚动的紧凑桌面表格、底部浮动操作栏（选中后出现）、整理弹窗、合并弹窗、查看合并来源弹窗
