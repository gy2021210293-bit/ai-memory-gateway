...
**认知模型**
- **三元一场** — 四格认知面板：用户核心 / 自我核心 / 关系核心 / 当前领域。原子认知卡 + 分层（明确陈述/演绎推断/归纳推断）+ 强化/取代生命周期，AI 提草稿 + 逐卡人工确认保存，不自动写库
...
四格认知面板，以**原子认知卡**维护（每格可有多张 active 卡）：
- **用户核心**（user_core）：用户的身份、价值、承诺、能力边界与成长理解
- **自我核心**（self_core）：AI 自身的定位与原则
- **关系核心**（relationship_core）：双方的关系基调与边界
- **当前领域**（current_field）：当前正处的事项，默认 14 天复核一次，过期后注入带「可能已过期」提示

每张认知卡带有：
- **层级**（`level`）：`explicit` 明确陈述 / `deductive` 演绎推断 / `inductive` 归纳推断。单次事件不得归纳为长期倾向；归纳推断建议置信度 ≤ 0.6
- **强化次数**（`times_derived`）：同一认知再次被证据支持时累计
- **生命周期**：新建（create）/ 强化（reinforce，不新增卡，次数 +1、合并证据）/ 取代（supersede，旧卡留档并记录 `supersedes` 指针，历史可追溯）
- **证据链**：`evidence_memory_ids` 指向来源记忆

AI 可以一次性提出各区块的原子卡草稿（含层级与生命周期建议），但**必须逐卡人工确认保存**，不自动写库。注入聊天上下文时按「层级优先级 → 强化次数 → 新近度」每格编译最多 3 张卡，只限数量不截断内容。

**证据回环**：每次人工决策（确认新建/强化/取代、手动修正、删除、拒绝草稿）都会写入 `cognitive_revision_log`，下次审视时作为额外证据回喂草稿 prompt——被人工拒绝或删除的认知不再被重新提出，被修正的认知以修正后版本为准，dashboard 底部可查看最近记录。
