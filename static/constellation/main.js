...
import { initRender, resizeRender, drawFrame, hitTest, onDataLoaded, resetCamera, zoomBy, panBy, rebuildLayouts } from './render.js';

const mc = document.getElementById('mc');
const $ = id => document.getElementById(id);
let hovered = null, dragging = false, dragged = false, lastX = 0, lastY = 0;

function showPanel(item, category) {
    $('p-cat').textContent = category;
    $('p-title').textContent = item.title || item.name || 'Memory';
...
onViewChange(() => { resetCamera(); rebuildLayouts(); renderBreadcrumb(); updatePills(); hidePanel(); });
...
mc.addEventListener('click', event => {
    ...
    else if (hit.type === 'star') { gotoStar(hit.id, view.conId); showPanel(hit.star, 'Memory fragment'); }
