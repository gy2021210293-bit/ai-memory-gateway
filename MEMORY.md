...
- Zero-event consolidation is not caused by absolute-date fallback once fragments were fetched. Consolidation now reads `reasoning_content` when `content` is empty, accepts in-range numeric string IDs, requires every meaningful singleton or grouped fragment to be represented, and reports returned/invalid-ID/empty-content event counts in logs, API status, and Dashboard.
- LLM JSON extraction must not accept an illustrative `[]` in reasoning before the final answer. It selects the final non-empty top-level array while skipping nested `merged_ids`; a genuinely empty result triggers one explicit regeneration, while both initial and retry outputs retain the 2,000-token ceiling and each event remains constrained to 80-180 Chinese characters and at most two sentences.
