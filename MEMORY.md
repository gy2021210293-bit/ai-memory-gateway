...
- Consolidated event narration is enforced as AI first person at the persistence boundary: the non-quoted narrative must use `我` for the AI. Third-person outputs are rejected without archiving their source fragments, and the consolidation prompt does not hardcode an AI name.
